...
#define SRAM_START  0x20000000U
#define SRAM_SIZE   (128*1024)//128K
#define estack    ((SRAM_START)+(SRAM_SIZE))
#define STACK_START (estack)
#define _Min_Stack_Size (2*1024)
...
//re-initalisation of sp is done .when the sp is needed to point to a location outside the internal mem.
void Reset_Handler(void)
{
	//Flash_vtor = (0x8000U)<<9;
	
	uint32_t size = (uint32_t)&_edata - (uint32_t)&_sdata;
	uint32_t Iterator = 0U; 
	uint8_t *SrcPtr = (uint8_t*)&_etext; /*start of .data in flash*/
	uint8_t *DesPtr = (uint8_t*)&_sdata; /*start of .data in sram*/
	...
	/*2-intialise .bss with zero ---> in ram*/
	size = (uint32_t)&_ebss - (uint32_t)&_sbss;
	DesPtr = (uint8_t*)&_sbss; /*start of .bss in sram*/
	...
	/*3-call init function from standard lib*/
...
//void Default_Handler(void) __attribute__((section(".Default_vector")));
